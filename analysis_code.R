...
drug_data <- read_xlsx("Medication Table.xlsx", col_names = T)
...
## and bone metabolism每related laboratory variables
...
## Outcome variablesㄩAll-cause mortality and refracture
...
## Figure 2A. Kaplan每Meier survival analyses stratified by nhypertension status
coxdata<-data.frame(survivaltime=survivaltime,death=death,hypertension=hypertension,gender=gender,age=age,BMI=BMI,smoke=smoke,drink=drink,
                    potassium=potassium,phosphorus=phosphorus,calcium=calcium,hemoglobin=hemoglobin,
                    CA=CA,VD1=VD1,VD2=VD2,ZOL=ZOL,CAL=CAL,ALN=ALN,CCI=CCI,
                    osteocalcin=osteocalcin,maindiagnosis=maindiagnosis)
...
## Figure 2B. Kaplan每Meier refracture analyses stratified by nhypertension status
...
## Figure 2. Kaplan每Meier survival and refracture analyses
...
hypertension <- ifelse( indicator == 1, 1, 0)  # 躺勛狻
...
coxdata<-data.frame(survivaltime=survivaltime,death=death,hypertension=hypertension,gender=gender,age=age,BMI=BMI,smoke=smoke,drink=drink,
                    ...
                    maindiagnosis=maindiagnosis)
coxdata <- coxdata[!is.na(coxdata$hypertension), ]
coxdata$age69 <- ifelse(coxdata$age <= 69, 0, 1)
coxdata$BMI23 <- ifelse(coxdata$BMI <= 23, 0, 1)
coxdata$CCI0 <- ifelse(coxdata$CCI == 0, 0, 1)
imp <- mice(coxdata, m = 5, maxit = 25, method = 'pmm', seed = 20251222)

## 
fit.sur.int <- with(imp,coxph(Surv(survivaltime, death) ~ hypertension * gender +  
                                hypertension * age69 +  hypertension * BMI23 + smoke + drink +potassium + phosphorus + calcium + hemoglobin +
                                CA + VD1 + VD2 + ZOL + CAL + ALN +  hypertension * CCI0 +osteocalcin +  hypertension * maindiagnosis ))
res <- summary(pool(fit.sur.int))
fit.sur.noint <- with(imp,coxph(Surv(survivaltime, death) ~ hypertension * gender +   
                                  hypertension * age69 +  hypertension * BMI23 + smoke + drink +potassium + phosphorus + calcium + hemoglobin +
                                  CA + VD1 + VD2 + ZOL + CAL + ALN +  hypertension * CCI0 +osteocalcin +  maindiagnosis ))
p_gender <- sprintf("%.3f",res$p.value[res$term == "hypertension:gender"])
p_age    <- sprintf("%.3f",res$p.value[res$term == "hypertension:age69"])
p_BMI    <- sprintf("%.3f",res$p.value[res$term == "hypertension:BMI23"])
p_CCI    <- sprintf("%.3f",res$p.value[res$term == "hypertension:CCI0"])
p_frac   <- sprintf("%.3f",D1(fit.sur.int, fit.sur.noint)$result[4])
interaction_p <- data.frame(
  Subgroup = c("Gender","Age (≒69 vs >69)","BMI (≒23 vs >23)", "CCI (0 vs ≡1)","Fracture category"),
  P_interaction = c(p_gender,p_age, p_BMI, p_CCI,p_frac))
interaction_p 
...
plot_df <- read_xlsx("sub_surv.xlsx", col_names = T)
plot_df$"P for interaction" <- as.character(plot_df$"P for interaction")
...
Figure.S4 <- forest(
  ...
  ref_line = 1, 
  xlim = c(0.1, 4))
Figure.S4 <- edit_plot(Figure.S4, col = 1, which = "text", 
...
imp <- mice(coxdata, m = 5, maxit = 25, method = 'pmm', seed = 20251222)
## 
fit.sur.int <- with(imp,coxph(Surv(refracturetime, refracture) ~ hypertension * gender +  
                                hypertension * age69 +  hypertension * BMI23 + smoke + drink +potassium + phosphorus + calcium + hemoglobin +
                                CA + VD1 + VD2 + ZOL + CAL + ALN +  hypertension * CCI0 +osteocalcin +  hypertension * maindiagnosis ))
res <- summary(pool(fit.sur.int))
fit.sur.noint <- with(imp,coxph(Surv(refracturetime, refracture) ~ hypertension * gender +   
                                  hypertension * age69 +  hypertension * BMI23 + smoke + drink +potassium + phosphorus + calcium + hemoglobin +
                                  CA + VD1 + VD2 + ZOL + CAL + ALN +  hypertension * CCI0 +osteocalcin +  maindiagnosis ))
p_gender <- sprintf("%.3f",res$p.value[res$term == "hypertension:gender"])
p_age    <- sprintf("%.3f",res$p.value[res$term == "hypertension:age69"])
...
Figure.S5 <- edit_plot(Figure.S5, col = 1, which = "text", 
                       x = unit(0.02, "npc"), hjust = 0)        
Figure.S5
